...
The live site is served by GitHub Pages from the `main` branch at `aeternasidera.com`. Merging to `main` publishes the changed files. The legacy `hello@antandros.space` mailbox remains in use during the email transition. The temporary site does not load third-party analytics or tracking scripts.
